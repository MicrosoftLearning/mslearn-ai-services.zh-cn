...
> **注意:**  
> 在 Azure 门户中看到容器显示为“正在运行”**并不**总是意味着它已准备好处理请求。 如果你尝试使用 `curl` 并遇到如下错误：
>
> ```text
...
> ```
>
> 这可能意味着容器尚未使用正确的参数或环境启动，或者缺少模型文件，或无法访问用于下载模型的 Internet。

### 另一种方式：使用 Azure CLI 部署容器

你可以尝试以下命令（将`rg-name`、`container-name``ApiKey` 和 `Billing` 终结点替换为你的值）：
